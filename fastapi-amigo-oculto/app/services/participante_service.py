...
class ParticipanteService:
    def __init__(self, participante_repository: BaseParticipanteRepository, 
                 user_repository: BaseUserRepository, grupo_repository: BaseGrupoRepository):
    ...
    async def registrar_participante(self, grupo_id: int, user_id: int) -> Participante:
        ...
        if grupo.administrador != user_id:
            raise ValueError("Você não tem permissão para adicionar participantes a este grupo.")

        await self.participante_repository.save(novo_participante.nome, novo_participante.id_user, grupo_id)
    ...
    async def quem_presentear(self, id_grupo: int, id_participante: int, id_user: int):
        participante: Participante = await self.participante_repository.get_by_id(id_participante)
        ...
        amigo_oculto: Participante = await self.participante_repository.get_by_id(amigo_oculto_id)
